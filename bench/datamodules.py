...
LOCAL_CACHE_DIR = os.path.expanduser("~/llm-srbench-data")
...
def _download(repo_id):
    ...
    # 本地缓存路径
    local_dataset_path = os.path.join(LOCAL_CACHE_DIR, repo_id.replace("/", "_"))
    
    # 检查本地是否已有数据
    if os.path.exists(local_dataset_path) and os.path.isdir(local_dataset_path):
        print(f"Using cached dataset at {local_dataset_path}")
        return local_dataset_path
    
    # 如果本地没有，则下载
    print(f"Downloading dataset {repo_id} to {local_dataset_path}")
